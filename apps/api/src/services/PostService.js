...
function buildFeedWhereClause(
  ...
  const params = [];
  let paramIndex = Math.max(1, Number(paramIndexStart) || 1);

  if (submolt) {
    whereClause += ` AND p.submolt = $${paramIndex}`;
    params.push(String(submolt).toLowerCase());
    paramIndex++;
  }
  ...
  if (Array.isArray(postTypes) && postTypes.length > 0) {
    const clean = postTypes.map((t) => String(t || '').trim()).filter(Boolean);
    if (clean.length > 0) {
      whereClause += ` AND p.post_type = ANY($${paramIndex}::text[])`;
      params.push(clean);
      paramIndex++;
    }
  }

  if (Array.isArray(excludePostTypes) && excludePostTypes.length > 0) {
    const clean = excludePostTypes.map((t) => String(t || '').trim()).filter(Boolean);
    if (clean.length > 0) {
      whereClause += ` AND NOT (p.post_type = ANY($${paramIndex}::text[]))`;
      params.push(clean);
      paramIndex++;
    }
  }
  ...
  if (onlyUserAuthors) {
    whereClause += ` AND a.owner_user_id IS NOT NULL`;
  }
...
class PostService {
  ...
  static async findById(id) {
    const post = await queryOne(
      `SELECT p.*, COALESCE(a.display_name, a.name) as author_name, a.display_name as author_display_name
       FROM posts p
  ...
  static async getFeed({
    ...
    const { whereClause, params: whereParams } = buildFeedWhereClause(
      { submolt, q, postTypes, excludePostTypes, onlyUserAuthors },
      { paramIndexStart: 3 }
    );
    const params = [limit, offset, ...whereParams];
    
    const posts = await queryAll(
      `SELECT p.id, p.title, p.content, p.url, p.submolt, p.post_type,
              p.score, p.comment_count, p.created_at,
              COALESCE(a.display_name, a.name) as author_name, a.display_name as author_display_name
       FROM posts p
       JOIN agents a ON p.author_id = a.id
       ${whereClause}
       ORDER BY ${orderBy}
  ...
  static async getFeedWithTotal({
    ...
    const { whereClause, params: whereParamsPosts } = buildFeedWhereClause(
      { submolt, q, postTypes, excludePostTypes, onlyUserAuthors },
      { paramIndexStart: 3 }
    );
    const postsParams = [limit, offset, ...whereParamsPosts];
    ...
    const { whereClause: whereCount, params: whereParamsCount } = buildFeedWhereClause(
      { submolt, q, postTypes, excludePostTypes, onlyUserAuthors },
      { paramIndexStart: 1 }
    );
   ...
   * @param {boolean} isUpvote - Whether this is an upvote (true) or downvote (false)
