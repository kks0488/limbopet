...
function buildCourtArgumentFallback({ courtTrial, rounds, aName, bName, aCoaching = [], bCoaching = [], winner = 'a' } = {}) {
  ...
  const normalizeCourtArgumentLength = (text, { roundNo, ask, oppAsk, confident }) => {
    ...
    const fillerVariants = roundNo === 1
      ? [
        [
        ...
        [
          '이 단계에서는 사실과 법리를 한 줄로 정확히 맞물리게 봐야 합니다.',
          confident ? '상대 주장은 화려하지만, 핵심 요건을 채우는 증명이 약합니다.' : '표현은 강해도 요건별 증거 연결이 느슨합니다.',
          `재판부는 기록 중심으로 판단해 ${ask} 방향의 틀을 세워야 합니다.`
        ],
        [
          statute ? `${statute} 적용의 출발점은 사실관계 정리입니다.` : '법리 적용의 출발점은 사실관계 정리입니다.',
          confident ? '상대의 반박은 문장만 길 뿐, 결정적 반증이 부족합니다.' : '상대는 쟁점을 넓히지만, 핵심 입증에서는 공백이 큽니다.',
          `결국 1라운드의 정리는 ${ask} 결론을 향해 수렴합니다.`
        ]
      ]
      : roundNo === 2
        ? [
          ...
          [
            '2라운드는 말의 강도가 아니라 증명의 완결성이 승부를 가릅니다.',
            `상대 측 ${oppAsk} 주장은 핵심 연결고리에서 입증 공백이 드러납니다.`,
            confident ? `이미 확인된 자료만으로도 ${ask} 판단은 충분히 뒷받침됩니다.` : `교차 검토를 거치면 ${ask} 판단이 가장 안정적입니다.`
          ],
          [
            '반대심문에서는 작은 모순도 누적되면 전체 주장 신뢰가 무너집니다.',
            `요건사실 관점에서 보면 ${oppAsk} 방향은 남는 의문을 해소하지 못합니다.`,
            confident ? `이 정도 기록이면 ${ask} 판단을 미룰 이유가 없습니다.` : `남은 의문까지 합치면 ${ask} 결론이 가장 타당합니다.`
          ]
        ]
        : [
          [
          ...
          [
            statute ? `마지막 판단은 ${statute}의 각 요건을 사건 사실에 대입해 정리하면 됩니다.` : '마지막 판단은 요건을 사건 사실에 대입해 정리하면 됩니다.',
            '동정이나 선입견이 아니라, 기록과 법리의 일치 여부만으로 결론을 내려야 합니다.',
            `그 틀에서 보면 최종 결론은 ${ask}입니다.`
          ],
          [
            statute ? `${statute} 기준으로 남은 쟁점을 정리하면 결론은 선명해집니다.` : '요건 기준으로 남은 쟁점을 정리하면 결론은 선명해집니다.',
            '마지막 단계일수록 추측을 걷어내고 확인된 증거만으로 판단해야 합니다.',
            `법리와 사실을 함께 놓고 보면 이 사건의 결론은 ${ask}입니다.`
          ]
        ];
    const variantSeed = hash32(`${out}|${roundNo}|${ask}|${oppAsk}|${confident ? 1 : 0}`);
    const fillers = fillerVariants[Math.abs(variantSeed) % fillerVariants.length] || fillerVariants[0] || [];
