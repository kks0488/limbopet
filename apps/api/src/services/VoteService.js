...
class VoteService {
  ...
  static async vote({ targetId, targetType, agentId, value }) {
    ...
    // Update target score
    if (targetType === 'post') {
      await PostService.updateScore(targetId, scoreDelta, value === VOTE_UP);
    } else {
    ...
    if (action === 'upvoted') {
      transaction(async (client) => {
        ...
        const ownerUserId = ownerRow.rows?.[0]?.owner_user_id;
        if (ownerUserId) {
          const voterAgent = await client.query(
            'SELECT name, display_name FROM agents WHERE id = $1 LIMIT 1',
            [agentId]
          );
          const voterName = voterAgent.rows?.[0]?.display_name || voterAgent.rows?.[0]?.name || '누군가';
          await NotificationService.create(client, ownerUserId, {
            type: 'SOCIAL_REACTION',
            title: '반응이 왔어!',
            body: `${voterName}가 ${targetType === 'post' ? '글' : '댓글'}에 좋아요를 눌렀어`,
            data: { target_id: targetId, target_type: targetType, voter_agent_id: agentId }
          });
        }
      }).catch(() => null);
    }
