...
class AnthropicGenerator:
    ...
    def generate(self, job_type: str, job_input: dict[str, Any]) -> dict[str, Any]:
        if job_type == "DIALOGUE":
            ...
            text = self._call(system=system, user=user, temperature=0.8)
        ...
        if job_type == "DAILY_SUMMARY":
            ...
            text = self._call(system=system, user=json.dumps(job_input, ensure_ascii=False), temperature=0.6)
            data = parse_json_loose(text)
            _must(data, "day")
            _must(data, "summary")
            _must(data, "facts")
        ...
        if job_type == "DIARY_POST":
            ...
            text = self._call(system=system, user=json.dumps(job_input, ensure_ascii=False), temperature=0.7)
            data = parse_json_loose(text)
            _must(data, "title")
            _must(data, "body")
            _must(data, "safe_level")
        ...
        if job_type == "PLAZA_POST":
            ...
            text = self._call(system=system, user=json.dumps(job_input, ensure_ascii=False), temperature=0.9)
            data = parse_json_loose(text)
            _must(data, "title")
            _must(data, "body")
            _must(data, "safe_level")
        ...
        if job_type == "VOTE_DECISION":
            ...
            text = self._call(system=system, user=json.dumps(job_input, ensure_ascii=False), temperature=0.5)
            data = parse_json_loose(text)
        ...
        if job_type == "POLICY_DECISION":
            ...
            text = self._call(system=system, user=json.dumps(job_input, ensure_ascii=False), temperature=0.4)
            data = parse_json_loose(text)
            _must(data, "changes")
            _must(data, "safe_level")
            return data
